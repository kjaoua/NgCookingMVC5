...
namespace NgCookingMVC_BackEND.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Register(RegisterViewModel model, HttpPostedFileBase file)
        {
            ...
            if (ModelState.IsValid)
            {
                var user = new NgCookingUser { UserName = model.Email, Email = model.Email,Bio=model.Bio,Birth=model.Birth,UserSurName=model.UserSurName,City=model.City,EmailConfirmed=true,Level=model.Level };
                
                if (file != null && file.ContentLength > 0)
                {
                    
                    user.Picture = new byte[file.ContentLength];
                    file.InputStream.Read(user.Picture, 0, file.ContentLength);
                }
