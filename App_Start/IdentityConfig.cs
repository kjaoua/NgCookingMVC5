...
namespace NgCookingMVC_BackEND
{
    ...
    public class ApplicationUserManager : UserManager<NgCookingUser>
    {
        public ApplicationUserManager(IUserStore<NgCookingUser> store)
            : base(store)
        {
        }
        ...
        {
            var manager = new ApplicationUserManager(new UserStore<NgCookingUser>(context.Get<NgCookingDbContext>()));
            // Configurer la logique de validation pour les noms d'utilisateur
            manager.UserValidator = new UserValidator<NgCookingUser>(manager)
            {
            ...
            manager.RegisterTwoFactorProvider("Code téléphonique ", new PhoneNumberTokenProvider<NgCookingUser>
            {
                MessageFormat = "Votre code de sécurité est {0}"
            });
            manager.RegisterTwoFactorProvider("Code d'e-mail", new EmailTokenProvider<NgCookingUser>
            {
            ...
            if (dataProtectionProvider != null)
            {
                manager.UserTokenProvider = 
                    new DataProtectorTokenProvider<NgCookingUser>(dataProtectionProvider.Create("ASP.NET Identity"));
            }
            return manager;
        }
    }
    ...
    public class ApplicationSignInManager : SignInManager<NgCookingUser, string>
    {
        ...
        public override Task<ClaimsIdentity> CreateUserIdentityAsync(NgCookingUser user)
        {
